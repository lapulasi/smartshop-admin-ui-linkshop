  ...
  (function() {
      ...
      (function() {
          ...
          * @remark 	类型区分大小写。如果自定义事件名称不是以小写"on"开头，该方法会给它加上"on"再进行判断，即"click"和"onclick"会被认为是同一种事件。
          */
     ...
     var CityList =
          ...
          * {"<b>container</b>" : {String|HTMLElement} 提供界面方式展现的容器。如果此参数为空，则不提供界面方式，也没有cityclick的事件派发
          * <br />"<b>map</b>" : {BMap} 化的map对象，如果传入此参数，则用户点击界面中的城市时，可以直接帮助用户定位到地图的相关城市位置}
          *
          * @example <b>参考示例：</b><br />
     ...
     CityList.prototype._init = function () {
         if (this._opts.container) {
             this._initContainer(this._opts.map);
         }
     }
     ...
     CityList.prototype.getBusiness = function (business, cbk) {
         ...
         DataMgr.request(url,function(json){
             ...
             for (var i = 0, len = result.length; i < len; i++) {
                 ...
                 result[i]['geo'] = coordinateToPoints(result[i]['coordinate']);
             }
     ...
     CityList.prototype._initContainer = function (map) {
         ...
         baidu.on(provinceDom, "change", function(){
             ...
             if( s_value == 131 || s_value == 289 || s_value == 332 || s_value == 132){
                 ...
                 selectArea(s_value, boroughDom, level);
             }else{
                 selectArea(s_value, cityDom, level);
             }
             dispatchCityClick(this.options[this.selectedIndex], level);
         });
         ...
         baidu.on(boroughDom, "change", function(){
             ...
             var level = 13;
             selectArea(s_value, businessDom, level, 1)
             businessDom.innerHTML = "";
             dispatchCityClick(this.options[this.selectedIndex], level);
         });

         function dispatchCityClick(option, level){
             /**
              * 点击城市名时，派发事件的接口
         ...
         baidu.on(businessDom, "change", function(e){
             ...
             dispatchCityClick(this.options[this.selectedIndex], zoom);

         });

         selectArea(1, provinceDom, 8);
     }
